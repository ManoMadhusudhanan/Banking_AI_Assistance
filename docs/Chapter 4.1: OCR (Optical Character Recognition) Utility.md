...
1. **Image Processing**: It first cleans up the image (e.g., improves brightness, contrast, deskews it if it's tilted) to make the text clearer.
2. **Character Detection**: It then scans the image line by line, looking for patterns that resemble letters or numbers.
3. **Character Recognition**: Once it identifies a potential character, it compares it to a vast library of known characters (e.g., 'A', 'B', '1', '2').
4. **Text Output**: Finally, it combines these recognized characters into words and sentences, giving you a digital text version of what was in the image.
...
Our `Banking_AI_Assistance` uses a simple function called extract_text_from_image to perform the OCR magic. The Orchestrator or a specific module like the [Intent Detection System](https://github.com/ManoMadhusudhanan/Banking_AI_Assistance/blob/main/docs/Chapter%204%3A%20Intent%20Detection.md) calls this function when a user uploads an image.

You saw a peek of its usage in [Chapter 4: Intent Detection System](https://github.com/ManoMadhusudhanan/Banking_AI_Assistance/blob/main/docs/Chapter%204%3A%20Intent%20Detection.md) where the `detect_intent` function might use it:
```
...
| Input (User Uploads an Image) | Output (Extracted Text) |
|------------------------------|-------------------------|
| A photo of a bank statement line showing:<br>`"Current Balance: $1,500.00"` | `"Current Balance: $1,500.00"` |
| An image containing the text:<br>`"LOAN APPLICATION FORM"` | `"LOAN APPLICATION FORM"` |
| A screenshot displaying:<br>`"Transfer successful!"` | `"Transfer successful!"` |
...
<img width="70%" height="619" alt="image" src="https://github.com/user-attachments/assets/5bde1334-fcbf-4163-8c19-44bff4f7209f" />
...
- E: This imports the Image module from the `Pillow` library. `Pillow` is essential for working with image files in Python, allowing us to open them in a format that `pytesseract` can understand.
